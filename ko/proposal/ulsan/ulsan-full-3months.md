...
> **표준화할 수 있는 외부 침입 대응은 본 사업으로 제공하고, 기업별 문서보안·내부자 관리·맞춤형 정보보안 SI는 별도 사업으로 분리합니다.**
...
| 구분 | 지원내용 |
|---|---|
| 외부 공격면 보호 | 인터넷 공개 웹서비스, 외부 접근 서비스, 원격접속 경로 |
| 웹 침입 대응 | WAF, 웹 요청·응답 분석, 변형·제로데이 의심 공격 분석 |
| 서버 침해 대응 | 로그인, 명령, 프로세스, 파일, 권한변경과 외부통신 분석 |
| PC 침해 대응 | 악성코드, 랜섬웨어, 의심 프로세스와 자격증명 탈취 행위 분석 |
...
| 관제 | 국내 24×365 전문 보안관제와 SLA 기반 대응 |
...
| 제외영역 | 제외사유 |
|---|---|
| 문서보안·DRM·DLP 구축 | 기업별 문서체계와 업무절차에 대한 별도 설계·연동이 필요 |
| 내부자 감시·행위평가 | 인사·직무·업무맥락과 법적·노무적 검토가 필요한 별도 영역 |
| 정보등급·문서분류 체계 | 기업별 규정과 조직구조를 기반으로 한 컨설팅 사업 |
| IAM·PAM 전면 재설계 | 계정·권한·결재·인사시스템별 맞춤 SI가 필요 |
| 내부 업무프로세스 통제 | ERP·그룹웨어·생산·품질시스템별 개별 개발이 필요 |
| 개인정보·ISMS 등 인증 컨설팅 | 침해대응 플랫폼과 별개의 관리체계·컴플라이언스 사업 |
| 기업별 맞춤 애플리케이션 연동 | 표준 인터페이스를 벗어나는 개발은 기간·예산을 별도 산정 |
| OT·ICS 제어장비 직접 통제 | 생산안전과 가용성 검토가 필요한 별도 전문사업 |
| 백업시스템 신규 구축 | 기존 백업을 활용한 복구지원은 가능하나 백업 인프라 구축은 별도 |
...
## 2.1 외부 해커의 침입은 기업의 규모와 관계없이 발생합니다
...
## 2.2 기업별 자체 대응만으로는 24시간 방어가 어렵습니다
...
## 3.1 제품을 설치해도 침해대응 공백이 남을 수 있습니다
...
이는 표준 사이버 침해 대응 서비스가 아니라 기업별 관리 SI 사업입니다.
...
|---|---|---|
| 목표 | 제품 구축과 관리체계 개선 | 외부 공격의 탐지·차단·사고대응 |
| 대상 | 광범위한 정보보안 항목 | 웹·서버·PC·계정·원격접속 공격 |
| 구축 | 기업별 상이 | 표준 패키지 |
| 운영 | 기업 자체 대응 | 국내 24시간 전문 SOC |
| 분석 | 제품별 경보 | 공격 전체 흐름 상관분석 |
| 사고 | 발생 후 별도 요청 | 실시간 차단·격리·포렌식 |
| 성과 | 구축수량·교육횟수 | 탐지율·통보시간·차단·복구결과 |
| 확장성 | 기업별 SI로 확대 어려움 | 100개사에서 300개사로 표준 확장 |
...
|---|---:|---:|---:|---:|
| 1차 집중 실증확산 | 울산시 선정 100개사 | 2026년 10~12월, 3개월 | 월 1천만 원, 총 3천만 원 | 30억 원 |
| 차년도 본사업 | 연속지원·신규선정 총 300개사 | 12개월 | 연 1억 2천만 원 | 360억 원 |
| 전체 사업규모 | 100개사에서 총 300개사로 확대 | 총 15개월 | 단계적 적용 | 390억 원 |
...
| 평가영역 | 배점 | 평가내용 |
|---|---:|---|
| 외부 공격 노출도 | 30점 | 웹서비스, 외부포트, VPN, 원격접속, 외부 유지보수 경로 |
| 침해 시 운영영향 | 20점 | 업무중단, 생산·거래·고객서비스와 공급망 영향 |
| 현재 탐지·대응 공백 | 20점 | WAF·EDR·로그수집·24시간 관제·사고대응 부재 |
| 표준 구축 적합성 | 15점 | 에이전트·로그연계 가능성, 10월 구축 가능 여부 |
| 기업 협조의지 | 10점 | 담당자 지정, 긴급조치·포렌식·훈련 참여확약 |
| 업종·정책 확산성 | 5점 | 울산 주력산업 대표성과 확산 가능성 |
| **합계** | **100점** | － |
...
## 5.1 기업당 월 1천만 원 표준 패키지

| 지원항목 | 표준범위 예시 |
|---|---|
| 외부 웹서비스 | 인터넷 공개 웹서비스 1식의 WAF·웹 공격 탐지·차단 |
| 업무용 서버 | 최대 20대의 로그인·명령·프로세스·파일·권한·외부통신 분석 |
| 업무용 PC | 최대 100대의 악성코드·랜섬웨어·이상 프로세스 탐지 |
| 원격접속 | 표준 VPN·관리자·원격접속 로그 연계 |
| 통합분석 | 웹·서버·PC·계정 경보의 공격체인 상관분석 |
| 전문관제 | 국내 24×365 SOC의 공격 검증·통보·대응지원 |
| 사고대응 | 차단·격리·자동 포렌식·영향분석·복구지원 |
| 교육·훈련 | 담당자 교육과 1회 이상의 모의시나리오 또는 대응점검 |
| 대시보드·보고 | 기업별 대시보드, 울산시 대시보드, 월간·종합보고 |

웹서비스가 없는 기업은 서버·PC·원격접속 관제범위를 확대하는 방식으로 표준 지원한도 안에서 조정할 수 있습니다.

표준수량을 초과하거나 비표준 시스템 연동이 필요한 경우에는 RFP의 초과단가 또는 별도 사업으로 처리합니다.

## 5.2 월 지원금의 산정근거

| 비용영역 | 주요내용 |
|---|---|
| 진단·온보딩 | 외부 공격면 확인, 자산등록, 에이전트와 로그연계 |
| 플랫폼·인프라 | 기업별 분리환경, 로그수집·저장·검색·분석 |
| WAF·EDR·XDR | 웹·서버·PC·계정 공격 탐지·차단 기능 |
| 24시간 SOC | 공격 검증, 위험분류, 기업·울산시 통보 |
| 사고대응 | 격리·포렌식·영향분석·복구지원 |
| 교육·실증 | 담당자 교육, 공격시나리오와 대응절차 점검 |
| 사업관리 | 대시보드, SLA·KPI·기업협조 평가와 보고 |
...
| 통제 주권 | 울산시와 기업이 계정·정책·보존·반출·삭제를 통제 |
...
|---|---|---:|---:|---:|
| P1 긴급 | 침해 진행, 랜섬웨어, C2, 대량 외부전송 | SOC 확인 후 15분 이내 | 30분 이내 | 사전 승인범위 즉시, 그 외 60분 이내 협의 |
...
| 기술 개선조치 | 20점 | 외부노출·취약설정·반복 공격 개선 |
...
| 차수 | 기업 수 | 주요내용 |
|---|---:|---|
| 선행 | 20~25개사 | 대표 업종·환경 구축, 표준절차 검증 |
| 2차 | 25~30개사 | 선행결과 반영, 병렬확산 |
| 3차 | 25~30개사 | 웹·원격접속·고위험 기업 우선 |
| 4차 | 잔여기업 | 월말까지 정상수집과 대시보드 확인 |

## 13.3 2026년 11월 전체 관제
...
## 16.1 외부 해커 침입에 대한 실질적인 방어

울산시는 제품 설치수량이 아니라 실제 공격의 탐지·차단·격리·포렌식 결과를 확보합니다.

## 16.2 중소·중견기업의 24시간 대응 공백 해소

자체 SOC를 운영하기 어려운 기업도 국내 전문인력의 24시간 지원을 받습니다.

## 16.3 공통 공격의 지역 단위 가시성

여러 기업에서 반복되는 웹 공격, 계정공격, 악성코드와 랜섬웨어 경향을 확인할 수 있습니다.

## 16.4 울산 제조업의 IT·OT 경계 보호

원격유지보수·VPN·점프서버·관리자 PC 등 외부 해커가 생산환경으로 접근하는 경로를 우선 보호합니다.

## 16.5 공공예산의 측정 가능한 성과
...
문서보안·내부자 관리·맞춤 SI를 제외하여 범위확대, 일정지연과 예산부족을 방지합니다.

## 16.7 울산형 소버린 사이버시큐리티 모델

국내 플랫폼·국내 데이터·국내 전문관제·국내 사고대응을 기반으로 다른 지역에도 확산 가능한 표준 침해대응 모델을 확보합니다.

---

# 17. 제안드리는 의사결정

1. 사업명을 **울산광역시 기업 사이버 침해 대응 통합지원사업**으로 정의
2. 목표를 외부 해커의 침입 탐지·차단·사고대응으로 명확히 한정
3. 문서보안·내부자 감시·IAM/PAM·맞춤 SI를 사업범위에서 제외
4. 1차 사업을 100개사·2026년 10~12월·30억 원 규모로 추진
5. 기업당 월 1천만 원의 표준 패키지와 자산 상한 확정
6. 외부 공격 노출도와 표준 구축 적합성 중심으로 기업 선정
7. 국내 24시간 SOC와 PLURA-XDR 기반 통합 침해대응 제공
8. SLA와 실제 탐지·차단·포렌식 성과로 사업 평가
9. 미협조·방임 기업은 시정기회를 거쳐 지원중단·차년도 제외
10. 1차 성과를 기반으로 차년도 총 300개사·12개월·360억 원 규모로 확대
...
문서보안, 내부자 관리, 직무권한, 인사·결재와 업무프로세스 통제는 기업마다 구조가 달라 표준화가 어렵고, 기업별 시스템통합과 더 큰 비용이 필요합니다. 이를 외부 침입 대응사업에 함께 포함하면 사업의 핵심이 흐려지고 100개 기업의 구축과 운영이 지연될 수 있습니다.
