...
T-Proxy에서 제공하는 복호화 웹 트래픽과 WAF·Nginx·웹·호스트 로그를 PLURA-XDR에서 통합 분석하는 구조로 구성합니다. 웹 최초 침투뿐 아니라 침투 이후의 명령, 프로세스, 파일, 계정, 네트워크 행위를 연결하여 확인합니다.
...
3. 기존 WAF·EDR·SOC 운영 절차와 PLURA-XDR 연계
4. 우선 시스템부터 단계적으로 적용
...
> **기존 WAF를 우회한 AI 웹 공격을 탐지하고, 최초 침투 이후 LOLBAS·LOTL을 이용한 은밀한 명령 실행과 내부 확산까지 MITRE ATT&CK 기반으로 연결하여 차단·분석·포렌식하는 통합 대응체계**

LOLBAS·LOTL은 시나리오 2와 3의 핵심 검증 항목으로 포함하고, 크리덴셜 스터핑은 짧은 보조 시나리오로 추가하여 AI 변형 웹 공격, 침투 후 정상 도구 악용, 계정 공격까지 이어지는 대응 능력을 보여주는 구성을 권고합니다.
