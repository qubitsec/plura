...
공격자가 방화벽이나 VPN의 취약점을 악용하여 내부 서버로 직접 진입하는 시나리오도 가능하지만, 특정 보안장비의 실제 제로데이 공격을 PoC 환경에서 재현하는 것은 현실적으로 어렵습니다.
...
경계 보안체계가 제로데이 공격을 놓치거나 우회당한 이후에도 PLURA-XDR이 웹 요청·응답, 서버·PC의 명령·프로세스·파일·계정·네트워크 행위를 하나의 공격 체인으로 연결하여 탐지·차단·포렌식할 수 있는지를 입증하는 것이 핵심입니다.

## Why PLURA-XDR

어떤 글로벌 보안제품도 알려지지 않은 모든 제로데이 공격을 사전에 완벽하게 차단한다고 보장할 수는 없습니다.

PLURA-XDR은 기존 방화벽이나 EDR을 대체하는 것이 아니라, 경계 보안이 실패한 상황에서도 다음과 같은 다층 대응을 제공합니다.
...
따라서 이번 PoC는 특정 보안장비의 취약점을 재현하는 시험은 아니고, 미토스급 AI가 경계 보안체계를 통과하여 내부에 침투한 상황에서 PLURA-XDR이 실제 피해 확산을 어떻게 탐지하고 대응하는지를 검증하는 시험입니다.
