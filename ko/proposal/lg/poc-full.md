...
PLURA-XDR은 웹·서버·PC에서 발생한 원본 로그와 침해 증거를 기반으로, 탐지 정책과 대응 절차를 국내 환경에 맞게 지속적으로 고도화할 수 있는 보안 운영 주권을 제공합니다.

특정 해외 보안제품의 탐지정책과 업데이트에만 의존하지 않고, 국내에서 공격 데이터를 축적하며 AI 프롬프트, 탐지 기준, 차단 및 포렌식 체계를 직접 통제할 수 있다는 점이 PLURA-XDR이 제공하는 소버린 사이버시큐리티의 핵심입니다. 향후 LG EXAONE과 같은 국내 AI와 연계할 경우, 원본 보안 데이터와 AI 판단·대응 체계를 국내에서 통제하는 소버린 AI 사이버시큐리티로 확장할 수 있습니다.

- 웹 요청·응답 원문과 ChatGPT 연동을 통한 제로데이 의심 공격 판단
- AI 미탐 발생 시 시그니처·탐지 룰을 추가하지 않고 프롬프트 개선만으로 탐지 성능 고도화
- 시그니처 기반 알려진 공격 탐지와 AI 기반 변형·제로데이 공격 탐지의 독립 운영
...
### 시그니처 탐지와 AI 탐지의 역할 구분

PLURA-WAF의 시그니처·탐지 룰은 알려진 공격을 빠르고 일관되게 탐지·차단하는 역할을 수행합니다. ChatGPT 연동 AI 분석은 기존 시그니처로 정의되지 않은 변형·제로데이 의심 공격을 웹 요청·응답의 맥락을 기반으로 판단하는 역할을 수행합니다.

두 탐지 방식은 대체 관계가 아니라 병행하는 구조입니다. 다만 이번 PoC의 **AI 탐지 고도화 검증 구간에서는 시그니처와 탐지 룰을 동결**하고, ChatGPT가 놓친 공격에 대한 보완은 **프롬프트 개선만으로 수행**합니다.

이를 통해 탐지 성능 향상이 새로운 시그니처를 추가한 결과가 아니라, AI가 공격 의도·행위·결과를 판단하는 능력을 개선한 결과임을 확인합니다.
...
- 시그니처 변경 없이 프롬프트 개선만으로 AI 공격 탐지 고도화
- 웹서버에서의 최초 명령 실행
- 웹셸·악성파일 생성
...
ChatGPT 연동 분석에서 미탐이 발생한 경우에는 해당 검증 구간의 시그니처와 탐지 룰을 변경하지 않고, 미탐 원인과 공격 맥락을 반영한 프롬프트 개선만으로 재분석합니다. 이를 통해 시그니처 업데이트와 별개로 AI 공격 탐지를 지속적으로 고도화할 수 있는지를 확인합니다.
...
flowchart LR
    ...
    subgraph XDR["<b>PLURA-XDR</b>"]
        ...
        PROMPT["AI 탐지 고도화<br/><b>미탐 분석·프롬프트 개선</b><br/>시그니처 변경 없음"]
    ...
    class PACKET,EVENT,MITRE,DISCOVERY,EXFIL,AI,PROMPT,RESPONSE,DASH xdr;
    class SOC soc;
```

> T-Proxy의 실제 배치 위치와 미러링 방식은 LG의 네트워크 구성에 맞춰 조정합니다. T-Proxy는 복호화 웹 트래픽을 제공하는 역할로 한정하며, 메일 로그 분석은 이번 PoC 범위에 포함하지 않습니다.

> PLURA-WAF는 기존 시그니처와 탐지 룰로 알려진 공격을 우선 탐지·차단합니다. 기존 룰만으로 판단하기 어려운 변형·제로데이 의심 공격은 PLURA-XDR이 웹 요청·응답 원문을 ChatGPT와 연동하여 분석하고, 공격으로 판단된 결과를 PLURA-WAF 차단에 반영합니다.

> PoC 중 ChatGPT 연동 분석에서 미탐이 발생하면 해당 공격을 위한 신규 시그니처나 탐지 룰을 추가하지 않습니다. 기존 시그니처를 동결한 상태에서 프롬프트만 개선한 뒤 동일 공격과 별도의 변형 공격을 재시험하여, AI 탐지 고도화 효과를 검증합니다.
...
초기 ChatGPT 분석에서 미탐이 발생한 경우에는 시그니처나 탐지 룰을 추가하지 않고 프롬프트 개선만으로 동일 공격과 변형 공격의 탐지 성능이 향상되는지를 함께 검증합니다. 이 과정의 핵심은 시그니처 업데이트가 아니라 **AI가 공격을 판단하는 능력의 고도화**입니다.
...
flowchart LR
    ...
    F["비정상 데이터 조회·응답"]
    G["PLURA-XDR 웹 패킷 분석"]
    H["ChatGPT 연동 분석<br/><b>Prompt vN</b>"]
    J{"공격 탐지 여부"}
    K["미탐 원인 분석<br/><b>프롬프트 개선</b><br/>시그니처 변경 없음"]
...
- AI 고도화 검증 기간 동안 기존 시그니처·탐지 룰 동결
- ChatGPT 미탐 케이스의 판단 실패 원인 분석
- 신규 시그니처 추가 없이 프롬프트만 개선
- 개선 전·후 동일 공격의 재탐지 결과 비교
- 프롬프트에 직접 사용하지 않은 별도 변형 공격으로 일반화 성능 확인
- 정상 요청과 기존 탐지 케이스에 대한 오탐·회귀 여부 확인
...
| 단계 | 시그니처·탐지 룰 | ChatGPT 프롬프트 | 검증 내용 |
|---|---|---|---|
| 기준선 측정 | 기존 버전으로 동결 | Prompt v1 | 알려진 공격, 제로데이 의심 공격, 정상 요청의 초기 탐지 결과 측정 |
| 미탐 확인 | 변경 없음 | Prompt v1 | 미탐 공격의 요청·응답, 공격 목적과 판단 실패 원인 분석 |
| AI 개선 | 변경 없음 | Prompt v2 | 미탐 원인과 공격 맥락을 반영하여 프롬프트만 개선 |
| 재검증 | 변경 없음 | Prompt v2 | 동일 공격을 재시험하여 탐지·차단 여부 확인 |
| 일반화 검증 | 변경 없음 | Prompt v2 | 프롬프트 개선에 직접 사용하지 않은 변형 공격과 정상 요청으로 재평가 |
| 운영 검증 | 변경 없음 | 승인된 Prompt 버전 | 탐지 향상, 오탐, 회귀, 처리시간 및 차단 연계 결과 확인 |

프롬프트 개선 과정에서는 공격 문자열이나 특정 값만 기억하도록 만드는 방식이 아니라, 공격 의도, 입력과 출력의 관계, 비정상 행위와 데이터 노출의 근거를 판단하도록 개선합니다.

프롬프트 버전, 변경 사유, 적용 시점과 테스트 결과를 기록하여 개선 효과를 재현하고, 필요하면 이전 프롬프트로 롤백할 수 있도록 관리합니다.
...
> 초기 ChatGPT 분석이 놓친 공격에 대해 시그니처와 탐지 룰을 변경하지 않고 프롬프트만 개선한 후, 동일 공격과 별도 변형 공격의 탐지 성능이 향상되는지를 확인합니다. 이를 통해 시그니처 업데이트와 별개로 AI 공격 탐지를 고도화할 수 있음을 검증합니다.
...
- AI 고도화 검증 구간에서는 PLURA-WAF 시그니처와 탐지 룰을 동결
- 프롬프트 버전, 변경 사유, 적용 시점, 승인자와 테스트 결과를 기록
- 프롬프트 개선 전·후 동일 공격과 별도 변형 공격을 동일 조건에서 재시험
...
| 핵심 검증 | 시그니처 동결 상태에서 프롬프트 개선을 통한 AI 미탐 보완과 웹 응답 데이터 노출 탐지 | OS·패치·소프트웨어 탐색, MITRE ATT&CK, LOLBAS·LOTL 및 데이터 유출 탐지 | 최초 침투부터 Discovery·데이터 유출·대응까지 연결 |
| PoC 난이도 | 낮음 | 중간 | 높음 |
| 차별성 | 신규 시그니처 없이 프롬프트 개선으로 AI 공격 탐지 고도화 | 정상 도구 악용과 데이터 유출까지 포함한 행위 탐지 | ChatGPT·WAF·EDR 통합 대응 역량 |
...
| 시그니처 동결 | AI 고도화 검증 전·후 신규 시그니처와 탐지 룰 추가 0건 |
| 프롬프트 기반 미탐 개선 | 사전 합의한 ChatGPT 미탐 케이스를 프롬프트 개선 후 재탐지하고 판단 근거 제시 |
| AI 일반화 검증 | 프롬프트 개선에 직접 사용하지 않은 별도 변형 공격에서 탐지 성능 향상 확인 |
| AI 회귀·오탐 관리 | 기존 탐지 케이스의 성능 저하 없이 정상 요청 오탐이 사전 합의 기준 이하 |
| 프롬프트 재현성 | Prompt 버전·변경 이력·적용 결과를 기록하고 동일 조건에서 결과 재현 |
...
- 시그니처·탐지 룰 동결 내역과 변경 0건 증적
- Prompt v1·v2 등 프롬프트 버전별 변경 내용과 적용 사유
- ChatGPT 미탐 공격의 개선 전·후 탐지 결과 비교표
- 동일 공격·별도 변형 공격·정상 요청에 대한 일반화·오탐·회귀 결과
...
1. 시그니처 기반 탐지와 AI 기반 탐지의 역할·운영 절차 분리
2. ChatGPT 미탐·오탐 피드백 수집과 프롬프트 개선 절차 수립
3. 프롬프트 버전 관리, 검증, 승인, 배포와 롤백 체계 확정
4. 탐지 정책과 정상 관리 행위의 허용 기준 정리
5. 중요 공격과 데이터 유출에 대한 IP 차단·호스트 포렌식 플레이북 확정
6. WAF·EDR·SOC 운영 절차와 PLURA-XDR 연계
7. 중요 데이터와 유출 경로에 대한 우선순위 설정
8. 우선 시스템부터 단계적으로 적용
9. 탐지 결과를 기반으로 프롬프트·정책과 대응 절차를 지속 보완
...
- 웹 요청·응답 원문과 ChatGPT 연동을 통한 제로데이 의심 공격 판단
- 시그니처 업데이트와 별개로 프롬프트 개선을 통한 AI 탐지 고도화
- ChatGPT 미탐·오탐 분석과 프롬프트 버전별 개선 전·후 검증
- 프롬프트 변경 이력·승인·배포·롤백 관리
