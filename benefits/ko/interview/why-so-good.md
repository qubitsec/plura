...
프루라는 이러한 흐름을 실시간으로 감시하며 **각 단계를 끊어내는 방식으로 방어**합니다.

가상자산 해킹의 경우, **계정 탈취·세션 재사용·자동화된 거래 시도**가 핵심 수법입니다.
프루라는 로그인 위치, 기기, 브라우저 등의 변화를 실시간으로 학습해
새로운 환경에서 이루어지는 고액 송금이나 비정상 시간대의 대량 거래를 즉시 탐지합니다.
이상 징후가 발견되면 **자동으로 거래를 보류하고 추가 인증을 요구하며**,
필요 시 **세션을 강제 종료하거나 계정을 잠그고 API 키를 폐기**합니다.
이 덕분에 실제 핀테크 기업들은 **수초 내 거래 보류 및 재인증 절차**를 적용할 수 있습니다.

또 다른 축은 **웹 기반 공격**입니다. 공격자는 먼저 웹 애플리케이션 취약점을 이용해
악성 스크립트를 업로드하고(**웹셸 업로드**), 이를 통해 서버 내부 권한을 탈취합니다.
이후 내부 시스템으로 이동하며(**측면 이동, lateral movement**),
지속적으로 통신하며 정보를 빼내는 **지능형 지속 공격(APT)**으로 발전합니다.

프루라는 웹 요청의 **본문(POST Body)**과 업로드된 파일을 실시간으로 분석하여
비정상 명령어, 인코딩, 암호화 패턴 등을 식별하고,
이를 서버의 **감사정책(Audit Policy) 로그**와 연계해 공격자의 행동을 추적합니다.
웹셸 설치나 비정상 프로세스 생성, 권한 상승 등의 징후가 감지되면
해당 세션을 즉시 차단하고, 관련 계정 및 네트워크 경로를 함께 격리합니다.

결국 프루라는 **웹과 단말, 네트워크, 계정, 데이터 접근 로그를 모두 통합 분석**하여
공격이 시작된 초기 단계에서부터 침투 경로를 차단합니다.
이러한 통합 대응 구조 덕분에, 웹셸을 통한 침투나 가상자산 탈취 같은
복합형 공격조차 **단 몇 분 안에 탐지·차단할 수 있는 체계**를 완성했습니다.
