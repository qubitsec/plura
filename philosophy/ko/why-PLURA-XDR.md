...
2020년대에 들어서면서 보안의 핵심 패러다임은 **“모든 것을 의심하라”**는 원칙을 기반으로 한 **제로 트러스트 아키텍처**(ZTA, Zero Trust Architecture)로 전환되었습니다. 제로 트러스트는 내부 네트워크를 더 이상 신뢰하지 않고, 네트워크 내·외부의 모든 접근을 지속적으로 검증하며, 인증 및 권한 부여를 통해 엄격히 관리하는 보안 모델입니다.
...
1. **암호화된 트래픽의 분석 한계**  
   네트워크 기반 보안 제품인 침입 차단 시스템(IPS), 침입 탐지 시스템(IDS), 네트워크 탐지 및 대응(NDR)은 **암호화된 패킷을 제대로 분석하기 어려운 한계**가 있습니다. 이러한 장비들은 주로 암호화되지 않은 트래픽에서만 효과적으로 작동하며, 암호화 트래픽의 경우 **웹 방화벽이 더 효과적**입니다. [1]
