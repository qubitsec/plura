...
| **네트워크 차단 (에이전트 단독)**   | **문서상 근거 없음** → PoC 필요                                                | **미제공**(네트워크 격리는 Genian NAC 연동 기능)                            |
| **시스템 전원 종료(Shutdown)** | **공개 문서에 기능 명시 없음** → 미확인                                             | 미지원                                                           |
...
| 항목           | **PLURA‑EDR**                              | **Genian EDR**                             |
| ------------ | ------------------------------------------ | ------------------------------------------ |
| **자동 대응 범위** | 프로세스 Kill, 파일 Delete·격리 **(Shutdown 미확인)** | 프로세스 Kill, 파일 Delete·격리 **(Shutdown 미지원)** |
| **네트워크 격리**  | 에이전트 단독 **근거 없음**                          | 에이전트 단독 **미지원** (NAC 연동 시 가능)              |
...
| **Shutdown**       | 🟥 미확인                      | 🟥 미지원                      |
| **네트워크 격리(Agent)** | 🟥 근거 없음                    | 🟥 미지원 (NAC 필요)             |
...
* **PLURA‑EDR**는 프로세스 종료와 파일 격리에 대해 **다중 조건 기반 자동·수동 대응**을 지원하며, 정책 세분화 유연성이 높습니다. 다만 **시스템 전원 종료**·**에이전트 단독 네트워크 차단** 기능은 공개 자료에서 확인되지 않았으므로 PoC로 검증이 필요합니다.  
* **Genian EDR** 역시 핵심 자동 대응(프로세스 Kill, 파일 Delete·격리)을 제공하지만, **네트워크 격리**는 Genian NAC 연동 기능이며, 정책 구성은 템플릿 기반으로 상대적으로 단순합니다.  
* 두 제품 모두 고위험 탐지 → 자동 대응 흐름을 갖추고 있으므로, 실제 도입 시 **Shutdown 지원 여부·네트워크 격리 필요성·정책 세분화 수준**을 기준으로 테스트하시길 권장합니다.
