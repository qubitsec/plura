...
| **운영자 실효성**            | 로그 기반 *시각적 추적 → 수동 차단*                | 탐지와 동시에 *시각화 + 자동 대응* 가능    |
...
* **PLURA‑EDR**
  ...
  * 실시간 MITRE Matrix로 탐지 흐름을 시각화하고, 콘솔에서 ‘즉시차단’으로 대응할 수 있습니다.
  * 다만 **TTP→정책 자동 차단** 기능은 문서에 구체적으로 명시돼 있지 않으므로, 실제 PoC에서 자동화 레벨을 확인하시는 것이 좋습니다.
...
아래는 **팩트 체크 결과**를 반영하여 다시 작성한 “2. 로그 수집 및 분석 범위 비교” 섹션입니다.
(근거가 공개 문서로 확인되지 않은 항목은 “미확인”, “가이드 제공” 등으로 완화했습니다.)
...
| **ETW 활용 여부**        | **미확인**(공개 문서에 명시 없음)                                                                            | **지원** – “Collect Windows Events (ETW)” 옵션 제공    |
| **Sysmon 로그 처리**     | **지원** – Sysmon 설치·수집·분석 가이드 제공                                                                  | **미지원** – Sysmon 언급 없음(자체 행위 이벤트로 대체)            |
| **로그 수집 채널 자동 구성**   | PowerShell·보안 채널 등 **수동/스크립트**로 활성화                                                              | 정책 설정 시 해당 채널을 **반자동** 활성화                       |
| **로그 분석 방식**         | MITRE TTP 룰 기반 상관 분석 → 실시간 탐지                                                                    | IOC·ML·XBA(행위) 엔진 → 실시간 이상행위 탐지                  |
| **포렌식 로그 추적 가능성**    | 원본 로그 보존 + 타임라인 재구성 기능 → 상태 복원·정밀 포렌식 지원                                                         | 이벤트·아티팩트 조사 지원(전체 상태 비교·복원 기능은 제한)               |
...
**이상 징후 발생 이후**, 시스템 변경 내역을 비교·분석하여 침해 여부를 검증하는 **상태 비교 기반 포렌식 기능**의 유무와 정밀도에 초점을 맞추어 비교합니다.
